---
title: Desarrollo de un Workflow con Snakemake
date: today
date-format: "DD MMMM, YYYY"
author: 
  Mathias Mangino
abstract:
  "De la Visualización a la Reproducibilidad: Ciencia de Datos en Bioinformática, 2025" 
...
## Descripción del workflow y ejecución Snakemake

El workflow implementado se organiza mediante Snakemake, un sistema de gestión de flujos reproducibles el cual se utiliza ampliamente para los analisis bioinformáticos. Snakemake permite estructurar un conjunto de tareas dependientes entre sí, garantizando reproducibilidad, escalabilidad y actualización automática de resultados [@kosterSnakemakeScalableBioinformatics2012]. El enfoque principal se basa en describir qué archivos deben ingresar y cuales deben obtenerse y dejar que Snakemake determine cómo generarlos, ejecutando solamente los pasos necesarios en función de las dependencias entre reglas.

En este contexto, el presente workflow se crea a partir algunos resultados que se desprenden de mi pryecto de maestría, los cuales son utilizados durante el proyecto para crear un sitio web. Sin enbargo, no se incluye la totalidad de los análisis originales.

Este workflow consta de 8 `rules`, una `rule` es una unidad que define un paso en el workflow. Se le debe especificar al menos un `input`, `output` y `comando` o `función` que debe ejecutar. Es similar a una función ya que encapsula y se le debe pasar ciertos argumentos, pero tiene un proposito distinto a una función. Las reglas que utilizamos son:
...
Por otro lado, generamos una lista completa de fastas con **expand()**, haciendo uso de la variable "`{fname}`" se genera la lista guardandola en la variable "`FASTA`". Ademas, para poder reutilizar una misma regla se uso parámetros como el tipo de análisis de frecuencia, que se extraen directamente desde el archivo de configuración, manteniendo el pipeline flexible, reproducible y fácil de extender.
...
El workflow concluye con la compilación de un reporte en formato HTML mediante la herramienta Quarto. Snakemake supervisa que tanto los resultados numéricos como las figuras necesarias estén disponibles previo a la generación del reporte, dado que el reporte hace uso de todo. Gracias al manejo automático de dependencias, cualquier modificación en los datos de entrada o en los pasos previos resulta en la actualización del documento final, asegurando consistencia entre análisis y reporte.

En conjunto, este o cualquier pipeline pueden obtener ventajas significativas haciendo uso de Snakemake como herramienta para gestionar los flujos de trabajo y lograr análisis reproducibles, modulares y eficientes. La definición explícita de reglas, entradas (`input`) y salidas (`outputs`) permite automatizar la ejecución ordenada del flujo de trabajo, paralelizar tareas independientes y actualizar solo los pasos necesarios cuando cambian los datos. El uso de pixi en este contexto es fundamental para evitar diferencias en los resultados o errores de ejecucion, ya que permite una gestion simple y rapida de los entornos. Finalmente, permite una integración muy simple de las herramientas externas como seqkit, infoseq, R y Quarto, haciendo que el pipeline sea flexible y extensible para otros análisis.
